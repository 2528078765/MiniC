...
1. 前往 [Releases](https://github.com/2528078765/MiniC/releases) 下载 `MiniC-0.0.1-windows-x64.exe`（版本 0.0.1）。
2. 安装包**只打包桌面端**，为单文件绿色安装包，图标使用 [icon/Log.png](icon/Log.png)。
3. **双击即可使用**：核心服务内嵌在桌面端进程内自动拉起，不需要命令行、不需要单独启动任何服务。
4. 首次使用进入「设置 → 模型设置」，填写 Provider（供应商名称）、Base URL、模型、API Key 并保存；配置明文保存在本机 `~/.minic/minic.json`，不入库、不上传。
5. 同样在「模型设置 → Embedding」配置向量化供应商（Provider 可自由填写，如 `dashscope` / `ollama`）、模型、Base URL 与 API Key，即可使用知识库功能。
