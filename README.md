...
1. 前往 [Releases](https://github.com/2528078765/MiniC/releases) 下载 `MiniC-Setup-0.0.1.exe`（版本 0.0.1）。
...
5. 知识库功能：同样在「模型设置 → Embedding」配置 Provider（自由填写，如 `dashscope` / `ollama`）、模型、Base URL 与 API Key。

### 方式二：绿色版（免安装）

- 下载 `MiniC-0.0.1-windows-x64.exe`，双击即用（不写注册表、无需安装）；其余使用方式同上。
...
ISCC.exe packaging\MiniC.iss                             # 生成安装包 dist\MiniC-Setup-0.0.1.exe（Inno Setup 6）
```
